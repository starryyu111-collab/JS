...
Starting from a trained FP32 model, we apply simulated quantization to weights and activations and evaluate the resulting fake-quantized forward pass. No model weights are updated, no quantization-aware training is performed, and the test set is not used for calibration. The proposed setting only changes the activation threshold selection rule during calibration.

\subsection{Simulated Quantization}
\label{subsec:quantization_setting}

For the INT4 post-ReLU activation experiments, activations are quantized as unsigned 4-bit values in the range $[0,15]$. Because the observed activation sites are post-ReLU modules, the clipping interval for activation site $l$ is $[0, \alpha_l]$. Given an activation value $a$, the simulated activation quantizer is
\begin{equation}
\begin{aligned}
