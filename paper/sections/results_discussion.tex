...
INT8-MinMax remains close to FP32, with 94.1100\% Top-1 accuracy compared with 94.1300\% for FP32. In contrast, INT4-MinMax drops to 88.2000\%, a 5.9300 percentage-point decrease. Both clipping variants alleviate this degradation: INT4-P99.9 reaches 92.9700\%, while INT4-MSE-Selected reaches 92.8600\%. Compared with INT4-MinMax, INT4-MSE-Selected improves Top-1 accuracy by 4.6600 percentage points and reduces the accuracy drop from 5.9300 to 1.2700 percentage points.

The result should be interpreted conservatively. INT4-MSE-Selected improves over INT4-MinMax and reduces reconstruction error, but it does not exceed the fixed INT4-P99.9 baseline in Top-1 accuracy in this run. Therefore, the experiment supports mitigation of INT4-MinMax degradation, not a claim that the proposed method is uniformly better than fixed percentile clipping.
...
Compared with INT4-MinMax, INT4-MSE-Selected reduces activation reconstruction MSE from 0.00273660 to 0.00034846 and logit MSE from 1.84912006 to 0.41728575. It also has slightly lower activation MSE and logit MSE than INT4-P99.9, although this lower reconstruction error does not translate into the highest Top-1 accuracy. This mismatch suggests that calibration reconstruction MSE is useful for analyzing quantization behavior but is not a complete substitute for test-set accuracy.

\subsection{Activation Histogram Discussion}
\label{subsec:activation_histogram_discussion}

An activation histogram helps explain why clipping can help low-bit activation quantization: rare large post-ReLU values can stretch the MinMax range, while high-percentile or MSE-selected clipping trades limited saturation for lower rounding error on common activation values. This work does not report a separate histogram figure, so the discussion is limited to this qualitative interpretation rather than a layer-specific numerical claim.
...
As visualized in Fig.~\ref{fig:layerwise_mse}, selected percentiles and reconstruction errors vary across observed post-ReLU activation sites. This supports the layer-wise nature of the proposed calibration strategy within the predefined candidate set. The result should not be interpreted as continuous globally optimal clipping, because the search is restricted to \(\{99.0,99.5,99.9,99.95,100.0\}\).
